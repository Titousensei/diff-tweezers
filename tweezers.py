...
class FoldingDiff:
    ...
    # ~ def get_fold_range(self, i):
        # ~ if not self.is_foldable(i):
            # ~ i = self.find_foldable_before(i)
        # ~ l = self.lines[i]
        # ~ try:
            # ~ if l.startswith(PREFIX_DIFF):
                # ~ pos = self.files.index(i)
                # ~ return i, self.files[pos + 1]
            # ~ elif l.startswith(PREFIX_CHUNK):
                # ~ pos = self.chunks.index(i)
                # ~ return i, self.chunks[pos + 1]
            # ~ except IndexError:
